 ...
 * between secid, and seci-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SECI_CONF_FILENAME));
    if (mode == HMM_SECID)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SECI_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/seci/seci>";
    const std::string URL_WEBSITE = "<https://seci.io>";

	//TODO-- add seci license
    return FormatParagraph(strprintf(_("Copyright (C) 2009 -%i The Bitcoin Core developers"), COPYRIGHT_YEAR)) + "\n" +
		   FormatParagraph(strprintf(_("Copyright (C) 2014 -%i The Dash Developers"), COPYRIGHT_YEAR)) + "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The SECI Core Developers"), COPYRIGHT_YEAR)) + "\n" + "\n" +
...
/** Sanity checks
 *  Ensure that seci is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single seci process is using the data directory.
    ...
    try {
       ...
       // Wait maximum 10 seconds if an old wallet is still running. Avoids lockup during restart
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. seci is probably already running."), strDataDir));
        if (probeOnly) {
            lock.unlock();
        }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", SECI_CONF_FILENAME)).string());
	...
	if (mapMultiArgs.count("-masternodepaymentskey")) // masternode payments priv key
    {
        if (!masternodePayments.SetPrivKey(GetArg("-masternodepaymentskey", ""))){
			...
			//LogPrintf("Masternode: Create new key %s", CseciSecret(secret).ToString());
   ...
   //if (!fDisableWallet) {
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("seci was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild seci with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty()) {
        return InitError(_("seci was not built with wallet support. Set -mineraddress, or rebuild seci with wallet support."));
    }
 #endif // !ENABLE_WALLET
    ...
    if (mapMultiArgs.count("-mineraddress")) {
        ...
        if (pwalletMain) {
            ...
            CseciAddress addr(GetArg("-mineraddress",""));
            CKeyID keyID;
            addr.GetKeyID(keyID);
