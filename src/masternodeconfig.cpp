...
bool CMasternodeConfig::read(std::string& strErr) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            CseciAddress address(donationAddress);
            if (!address.IsValid()) {
                strErr = "Invalid seci address in masternode.conf line: " + line;
                streamConfig.close();
                return false;
            }
        }
        ...
        if(port != 9819 && Params().NetworkIDString() == CBaseChainParams::MAIN) {
            strErr = "Invalid port detected in masternode.conf: " + line + " (must be 9819 for mainnet)";
            streamConfig.close();
            return false;
        }
        else if(port != 9829 && Params().NetworkIDString() == CBaseChainParams::TESTNET) {
            strErr = "Invalid port detected in masternode.conf: " + line + " (must be 9829 for testnet)";
            streamConfig.close();
            return false;
        }
