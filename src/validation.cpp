...
#if defined(NDEBUG)
# error "seci cannot be compiled without assertions."
#endif

/**
 * Global state
 */
#define PRE_SUBSIDY_HEIGHT 14400
...
const std::string strMessageMagic = "seci Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache, bool fDryRun)//TODO--
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s SECI additional fees, %d delta bytes\n",
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if(pindex->nHeight > 0) {
        ...
        if (!found) {
            if(fDebug) LogPrintf("[hamrex] ** rejecting block - treasury funding missing! **\n");
    ...
    if(pindex->nHeight > chainparams.GetConsensus().MasternodePaymentStartHeight + 7475)
    {
        ...
        else
        {
            ...
            CseciAddress address(txdestaddr);
