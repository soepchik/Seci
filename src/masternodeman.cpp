...
void CMasternodeMan::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        if(addr.GetPort() != 9819 && Params().NetworkIDString() == CBaseChainParams::MAIN) {
            LogPrintf("dsee - Got bad mainnet Masternode port: Actual:%d => Expected:%d\n", addr.GetPort(), 9819);
            return;
        
        }
        else if(addr.GetPort() != 9829 && Params().NetworkIDString() == CBaseChainParams::TESTNET) {
        ...
        if(AcceptableInputs(mempool, state, MakeTransactionRef(mtx))){
            ...
            // should be at least not earlier than block when 1000 SECI tx got MASTERNODE_MIN_CONFIRMATIONS
            ...
            {
                CBlockIndex* pMNIndex = (*mi).second; // block for 5000 SECI tx -> 1 confirmation
